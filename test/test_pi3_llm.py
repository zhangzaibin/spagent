...
def extract_video_frames(video_path: str, target_fps: float = 1.0) -> List[str]:
    ...
    cap.release()
    print(f"从视频中提取了 {len(frame_paths)} 帧 (时长: {total_duration:.2f}s, 原始fps: {original_fps:.2f}, 目标fps: {target_fps})")
    return frame_paths
...
result = agent.solve_problem(
    ...
    pi3_target_fps=5.0  # pi3工具使用更高的帧率
)
