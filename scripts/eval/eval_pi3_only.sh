...
# ============================================================
#  eval_pi3_only.sh
#  Run quick_eval.py with the Pi3 3D reconstruction tool only.
#
#  Tools enabled:
#    pi3 — pi3_tool: 3D reconstruction from images, renders
#          point clouds / novel viewpoints for spatial reasoning
#
#  Defaults : MindCube (per-category) + VSIBench, spatial prompt
#
#  Usage:
#    bash scripts/eval/eval_pi3_only.sh
#    MODEL=gpt-4.1 bash scripts/eval/eval_pi3_only.sh
#    DATASETS="MindCube" PER_CATEGORY=100 bash scripts/eval/eval_pi3_only.sh
#    DATASETS="VStarBench" PROMPT=general LIMIT=50 bash scripts/eval/eval_pi3_only.sh
#
...
eval_print_header "eval_pi3_only.sh" \
    "Tools     : pi3" \
    "Prompt    : ${PROMPT}" \
