...
```bash
python -m spagent.skills.run pi3_tool --args '{"image_path": ["assets/dog.jpeg"]}' --use-mock
```
...
Use `--server-url URL` to override the default backend (`http://127.0.0.1:20030`), and `--output-dir DIR` to redirect artifacts when the tool supports it.

## Runtime requirements

- Runtime class: **server**
- Server: `http://127.0.0.1:20030` (health check: `GET /health`)
- Launch: `python spagent/external_experts/Pi3/pi3_server.py --checkpoint_path checkpoints/pi3/model.safetensors --port 20030`
