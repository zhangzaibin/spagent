...
description: Generate a video from a text prompt (and optionally a reference image) using Alibaba Wan.
...
Generate a video from a text prompt (and optionally a reference image) using Alibaba Wan. Returns the path to the generated .mp4 video file. Use this when the task requires creating a video visualization, animation, or video content from a description. Supports text-to-video and image-to-video generation.
...
| `image_path` | string | no | — | Optional path or URL to a reference image for image-to-video generation. When provided, the model generates a video conditioned on this image. |
| `duration` | integer | no | 5 | Video duration in seconds (3–10). Default is 5. |
...
```bash
python -m spagent.skills.run video_generation_wan_tool --args '{"prompt": "a dog"}' --use-mock
```
