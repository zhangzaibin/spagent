...
This tool is suitable for motion and spatial reasoning tasks that involve camera movement, object rotation, or directional motion analysis. It performs 3D reconstruction from multiple images using VGGT (Visual Geometry Grounded Transformer) to generate point clouds and visualizations from CUSTOM viewing angles.
...
```bash
python -m spagent.skills.run vggt_tool --args '{"image_path": ["assets/dog.jpeg"]}' --use-mock
```
...
Use `--server-url URL` to override the default backend (`http://127.0.0.1:20032`), and `--output-dir DIR` to redirect artifacts when the tool supports it.

## Runtime requirements

- Runtime class: **server**
- Server: `http://127.0.0.1:20032` (health check: `GET /health`)
- Launch: `python spagent/external_experts/VGGT/vggt_server.py --port 20032`
