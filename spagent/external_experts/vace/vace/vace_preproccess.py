...
def main(args):
    ...
    if 'frames' in output_params:
        ...
        if frames is not None:
            ...
            if not save_one_video(save_path, frames, fps=save_fps):
                raise RuntimeError(
                    f"Failed to write preprocess video: {save_path}. "
                    "Check imageio/imageio-ffmpeg and OpenCV (see requirements-vace.txt)."
                )
    ...
    if 'masks' in output_params:
        ...
        if frames is not None:
            ...
            if not save_one_video(save_path, frames, fps=save_fps):
                raise RuntimeError(
                    f"Failed to write preprocess mask video: {save_path}. "
                    "Check imageio/imageio-ffmpeg and OpenCV (see requirements-vace.txt)."
                )
            print(f"Save frames result to {save_path}")
