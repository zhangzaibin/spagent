...
def save_one_video(file_path, videos, fps=8, quality=8, macro_block_size=None):
    """Write frames to MP4. Prefer imageio+ffmpeg; fall back to OpenCV if imageio has no backend."""

    def _save_with_cv2():
        if len(videos) == 0:
            raise ValueError("Empty video frame list.")
        h, w = videos[0].shape[:2]
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        writer = cv2.VideoWriter(file_path, fourcc, float(fps), (w, h))
        if not writer.isOpened():
            raise RuntimeError(
                f"cv2.VideoWriter could not open {file_path!r}; "
                "check that opencv was built with video codecs."
            )
        try:
            for frame in videos:
                if frame.ndim == 2:
    ...
    try:
        if imageio is not None:
            try:
                ...
                print(f"Video save error (imageio), falling back to OpenCV: {e}")
        _save_with_cv2()
        return True
    except Exception as e:
