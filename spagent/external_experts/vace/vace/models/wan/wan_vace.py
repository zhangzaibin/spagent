...
def _emit_vace_server_denoise_progress(step_idx: int, total: int) -> None:
    """Machine-parseable line for vace_server (streamed stdout). Format: VACE_SERVER_PROGRESS denoise <cur> <total>"""
    if os.environ.get("VACE_SERVER_PROGRESS"):
        print(f"VACE_SERVER_PROGRESS denoise {step_idx + 1} {total}", flush=True)
...
class WanVace(WanT2V):
    ...
    def generate(self,
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            for step_i, t in enumerate(tqdm(timesteps, desc="denoise", file=sys.stderr)):
                _emit_vace_server_denoise_progress(step_i, n_steps)
