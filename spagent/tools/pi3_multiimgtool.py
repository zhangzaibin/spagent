"""
Pi3 Multi-Image 3D Reconstruction Tool

This module contains the Pi3MultiimgTool that wraps
...
class Pi3MultiimgTool(Tool):
    """Tool for 3D reconstruction from multiple images of the same scene using Pi3"""
    ...
    def __init__(self, use_mock: bool = True, server_url: str = "http://localhost:20030"):
        ...
        """
        super().__init__(
            name="pi3_multiimg_tool",
            description="Perform 3D reconstruction from multiple images of the same scene to generate a single point cloud with multi-view visualizations and customizable viewing angles."
        )
            ...
            try:
                ...
                class SimpleMockPi3:
                    def infer_from_images(self, image_paths, azimuth_angle=None, elevation_angle=None, **kwargs):
                        ...
                        # Use first image from the list for naming, show count if multiple
                        first_image = image_paths[0] if image_paths else "mock"
                        image_name = Path(first_image).stem
                        if len(image_paths) > 1:
                            result_name = f"{image_name}_multi{len(image_paths)}"
                        else:
                        ...
                        result = {
                            "success": True,
                            "ply_filename": f"result_{result_name}.ply",
                            "points_count": 50000,
                            "camera_views": []
                        }
                        
                        # Add image count and angle information as text with larger font
                        if azimuth_angle is not None and elevation_angle is not None:
                            ...
                            draw.text((30, 30), f"Images: {len(image_paths)}", fill='black')
                            draw.text((30, 50), f"Azimuth: {azimuth_angle}°", fill='black')
                            draw.text((30, 70), f"Elevation: {elevation_angle}°", fill='black')
                            draw.text((30, 90), "Pi3 Multi Mock", fill='gray')
        ...
        return {
            ...
            "properties": {
                "image_paths": {
                    ...
                    "description": "List of paths to input images from the same scene for 3D reconstruction. Multiple images of the same scene will be used to generate a single, more accurate point cloud."
                },
                "azimuth_angle": {
                    "type": "number",
            ...
            "required": ["image_paths", "azimuth_angle", "elevation_angle"]
        }
    ...
    def call(
        ...
        Args:
            image_paths: List of paths to the input images from the same scene for 3D reconstruction
        ...
        try:
            ...
            if not image_paths or len(image_paths) == 0:
                return {
                    "success": False,
                    "error": "image_paths is required and must contain at least one image"
                }
            
            logger.info(f"Running Pi3 3D reconstruction on {len(image_paths)} images: {image_paths}")
            ...
            if missing_images:
                return {
                    "success": False,
                    "error": f"Image files not found: {missing_images}"
                }
            ...
            # Execute 3D reconstruction with multiple images
            result = self._client.infer_from_images(
                image_paths=image_paths,  # Pass the list of image paths directly
                conf_threshold=0.1,
                rtol=0.03,
                generate_views=True,
                use_filename=True,
            ...
            if result and result.get('success'):
                ...
                output_path = self._save_generated_images(result, image_paths)
    ...
    def _save_generated_images(self, result: Dict[str, Any], image_paths: List[str]) -> Optional[str]:
        """
        Save generated images from Pi3 result and return the path to the first saved image
        
        Args:
            result: Pi3 reconstruction result
            image_paths: List of original input image paths for naming
        ...
        try:
            ...
            # Generate base name from input images
            # Use the first image's name as base, append count if multiple images
            first_image_name = Path(image_paths[0]).stem
            if len(image_paths) > 1:
                base_name = f"{first_image_name}_multi{len(image_paths)}"
            else:
                base_name = first_image_name
            
            saved_images = []
            
            # Save all generated images (representing different camera views of the point cloud)
            for i, view_data in enumerate(camera_views):
                try:
                    ...
                    # Create filename with base name from multiple input images and camera view info
                    img_filename = f"pi3_{base_name}_cam{camera}_azim{azimuth}_elev{elevation}.png"
