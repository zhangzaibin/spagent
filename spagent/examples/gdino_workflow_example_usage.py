#!/usr/bin/env python3
"""
Grounding DINO QA工作流使用示例

这个脚本演示了如何使用Grounding DINO QA工作流来处理不同类型的图像问答任务。
"""
...
def example_gdino_questions():
    """Grounding DINO相关问题的示例"""
    return [
...
def run_example(workflow, image_path: str, questions: list, question_type: str):
    ...
    for i, question in enumerate(questions, 1):
        ...
        try:
            ...
            print(f"Grounding DINO工具使用: {result['gdino_used']}")

            if result['gdino_result']:
                print(f"任务类型: {result.get('task_type', 'unknown')}")
                print(f"处理后图像路径: {result['gdino_result'].get('output_path', 'N/A')}")
...
def main():
    ...
    # 运行Grounding DINO相关问题示例
    gdino_questions = example_gdino_questions()
    run_example(workflow, image_path, gdino_questions, "Grounding DINO-related")
