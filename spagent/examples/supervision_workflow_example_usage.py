#!/usr/bin/env python3
"""
SupervisionQA工作流使用示例

这个脚本演示了如何使用SupervisionQA工作流来处理不同类型的图像问答任务。
"""
...
def run_example(workflow, image_path: str, questions: list, question_type: str):
    ...
    for i, question in enumerate(questions, 1):
        ...
        try:
            ...
            print(f"Supervision工具使用: {result['supervision_used']}")
            
            if result['supervision_result']:
                print(f"任务类型: {result.get('task_type', 'unknown')}")
                print(f"处理后图像路径: {result['supervision_result'].get('output_path', 'N/A')}")
