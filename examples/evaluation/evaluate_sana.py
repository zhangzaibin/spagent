...
def load_jsonl_data(path: str) -> List[Dict[str, Any]]:
    """Load a JSONL file without extra dependencies (e.g., pandas)."""
...
def extract_sana_call_info(agent_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Newer SPAgent results store execution outputs in `tool_results` keyed as
    # "<tool_name>_iter<k>" (e.g., "image_generation_sana_tool_iter1"), while
    # `tool_calls` may only contain name/arguments without an attached `result`.
    ...
    if isinstance(tool_results, dict):
        ...
        if matching_keys:
            # Pick the latest iteration result
...
def evaluate_sana_config(
    ...
    summary = {
        ...
        "generation_successful_samples": len(generated),
        "generation_success_rate": (len(generated) / len(results)) if results else 0.0,
    ...
    print(
        f"Generation success rate: {summary['generation_success_rate']:.4f} "
        f"({summary['generation_success_rate'] * 100:.2f}%)"
    )
    print(f"Average inference time: {summary['average_inference_time']:.2f} seconds")
    print(f"Total inference time: {summary['total_inference_time']:.2f} seconds")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-4o",
        help="LLM model used by SPAgent",
    )
