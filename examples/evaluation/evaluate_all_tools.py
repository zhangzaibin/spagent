...
def _extract_image_call_info(agent_result: Dict[str, Any], tool_name: str) -> Dict[str, Any]:
    ...
    # SPAgent stores execution outputs in `tool_results` keyed as "<tool>_iter<k>"
    # (e.g., "image_generation_sana_tool_iter1"). `tool_calls` may contain only
    # name/arguments without an attached `result`, so we fall back to `tool_results`.
