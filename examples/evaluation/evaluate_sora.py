"""
Evaluation script for the Sora video generation tool via SPAgent.
...
Dataset format (JSONL, one JSON object per line):
{
    ...
    "duration": 10,            // optional, 5-20 seconds (default 10)
    "resolution": "1080p",     // optional, "480p" / "720p" / "1080p" (default "1080p")
    "aspect_ratio": "16:9"     // optional, "16:9" / "9:16" / "1:1" (default "16:9")
}
"""
...
TOOL_CONFIGS = {
    "sora_real": [
        SoraTool(use_mock=False),
    ],
    "sora_mock": [
        SoraTool(use_mock=True),
    ],
}
...
def evaluate_single_sora_sample(
    agent: SPAgent,
    sample: Dict[str, Any],
    image_base_path: str,
    config_name: str = "sora",
    max_iterations: int = 3,
    video_num_frames: int = 4,
) -> Dict[str, Any]:
    """Evaluate a single Sora generation sample.

    Args:
        agent: SPAgent instance equipped with SoraTool.
    ...
    try:
        ...
        # Extract Sora-specific information from tool calls
        sora_call_info = extract_sora_call_info(agent_result)

        # Determine overall generation success
        generation_succeeded = sora_call_info["called"] and sora_call_info["success"]

        task_data = {
            "id": sample_id,
            "prompt": prompt,
            "task": task_type,
            "sora_called": sora_call_info["called"],
            "generation_success": generation_succeeded,
            "output_path": sora_call_info.get("output_path", ""),
            "file_size_bytes": sora_call_info.get("file_size_bytes", 0),
        ...
        return {
            ...
            "output_path": sora_call_info.get("output_path", ""),
            "file_size_bytes": sora_call_info.get("file_size_bytes", 0),
            "sora_prompt_used": sora_call_info.get("sora_prompt", ""),
            "sora_duration": sora_call_info.get("duration", None),
            "sora_resolution": sora_call_info.get("resolution", None),
            "sora_aspect_ratio": sora_call_info.get("aspect_ratio", None),
            "sora_error": sora_call_info.get("error", None),
            "inference_time": inference_time,
...
def extract_sora_call_info(agent_result: Dict[str, Any]) -> Dict[str, Any]:
    """Extract Sora tool call information from an agent result.
    ...
    Returns:
        ...
        sora_prompt, duration, resolution, aspect_ratio, error.
    """
    info: Dict[str, Any] = {
        "called": False,
        "success": False,
    ...
    for call in tool_calls:
        if call.get("name") == "video_generation_sora_tool":
            info["called"] = True
            args = call.get("arguments", {})
            info["sora_prompt"] = args.get("prompt", "")
            info["duration"] = args.get("duration")
            info["resolution"] = args.get("resolution")
            ...
            break  # Only inspect the first Sora call
...
def evaluate_sora_config(
    ...
    """Run a full Sora evaluation for a given tool configuration.
    ...
    for sample in tqdm(data, desc="Evaluating"):
        result = evaluate_single_sora_sample(
            agent=agent,
            sample=sample,
    ...
    tool_usage_rate = sora_called_count / n_success if n_success else 0.0
    ...
    for r in successful_results:
        ...
        if task not in task_stats:
            task_stats[task] = {"total": 0, "sora_called": 0, "generation_success": 0}
        task_stats[task]["total"] += 1
        if r.get("sora_called"):
            task_stats[task]["sora_called"] += 1
        if r.get("generation_success"):
            task_stats[task]["generation_success"] += 1

    for task in task_stats:
        t = task_stats[task]
        t["tool_usage_rate"] = t["sora_called"] / t["total"] if t["total"] else 0.0
        t["generation_success_rate"] = t["generation_success"] / t["total"] if t["total"] else 0.0

    # Resolution distribution (Sora-specific)
    resolution_stats: Dict[str, int] = {}
    for r in successful_results:
        res = r.get("sora_resolution") or "unknown"
    ...
    return {
        ...
        "sora_called_count": sora_called_count,
...
def print_sora_evaluation_results(results: Dict[str, Any]):
    """Print a formatted summary of Sora evaluation results."""
    print(f"\n{'='*70}")
    print(f"Sora Evaluation Results — {results['config_name']}")
    print(f"{'='*70}")
    print(f"Model:                    {results['model']}")
    print(f"Total samples:            {results['total_samples']}")
    print(f"Successfully processed:   {results['successful_samples']}")
    print(f"Failed to process:        {results['failed_samples']}")
    print(f"Sora tool called:         {results['sora_called_count']}  "
    ...
    if results.get("task_statistics"):
        print(f"\n{'Task Breakdown':-<50}")
        print(f"{'Task':<15} {'Total':<8} {'Sora Called':<13} {'Gen Success':<14} "
              f"{'Usage Rate':<13} {'Success Rate'}")
        print("-" * 70)
        for task, stats in results["task_statistics"].items():
            print(
                f"{task:<15} {stats['total']:<8} {stats['sora_called']:<13} "
                f"{stats['generation_success']:<14} "
                f"{stats['tool_usage_rate']:<13.1%} "
                f"{stats['generation_success_rate']:.1%}"
            )

    if results.get("resolution_statistics"):
        print(f"\n{'Resolution Distribution':-<50}")
        for res, count in sorted(results["resolution_statistics"].items()):
            print(f"  {res:<10} {count}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate SPAgent with the Sora video generation tool."
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="dataset/sora_eval_data.jsonl",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel worker threads (default: 1; Sora API has rate limits).",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--use_mock",
        action="store_true",
        help="Use mock Sora service instead of the real OpenAI API.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    for config_name, tools in configs_to_run.items():
        ...
        print_sora_evaluation_results(results)

    # Save results to JSON
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = (
        f"sora_evaluation_results_{args.model.replace('-', '_')}_{timestamp}.json"
    )
