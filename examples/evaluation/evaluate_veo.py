"""
Evaluation script for the Veo video generation tool via SPAgent.
...
Dataset format (JSONL, one JSON object per line):
{
    ...
    "duration": 8,             // optional, 5 or 8 seconds (default 8)
...
def evaluate_single_veo_sample(
    agent: SPAgent,
    sample: Dict[str, Any],
    image_base_path: str,
    config_name: str = "veo",
    ...
    """Evaluate a single Veo generation sample.

    Args:
        agent: SPAgent instance equipped with VeoTool.
    ...
    try:
        ...
        # Extract Veo-specific information from tool calls
        veo_call_info = extract_veo_call_info(agent_result)

        # Determine overall generation success
        generation_succeeded = veo_call_info["called"] and veo_call_info["success"]

        task_data = {
            "id": sample_id,
            "prompt": prompt,
            "task": task_type,
            "veo_called": veo_call_info["called"],
            "generation_success": generation_succeeded,
            "output_path": veo_call_info.get("output_path", ""),
            "file_size_bytes": veo_call_info.get("file_size_bytes", 0),
        ...
        return {
            ...
            "output_path": veo_call_info.get("output_path", ""),
            "file_size_bytes": veo_call_info.get("file_size_bytes", 0),
            "veo_prompt_used": veo_call_info.get("veo_prompt", ""),
            "veo_duration": veo_call_info.get("duration", None),
            "veo_aspect_ratio": veo_call_info.get("aspect_ratio", None),
            "veo_error": veo_call_info.get("error", None),
            "inference_time": inference_time,
...
def extract_veo_call_info(agent_result: Dict[str, Any]) -> Dict[str, Any]:
    """Extract Veo tool call information from an agent result.
    ...
    Returns:
        ...
        veo_prompt, duration, aspect_ratio, error.
    """
    info: Dict[str, Any] = {
        "called": False,
        "success": False,
    ...
    for call in tool_calls:
        if call.get("name") == "video_generation_veo_tool":
            info["called"] = True
            args = call.get("arguments", {})
            info["veo_prompt"] = args.get("prompt", "")
            ...
            break  # Only inspect the first Veo call
...
def evaluate_veo_config(
    ...
    """Run a full Veo evaluation for a given tool configuration.
    ...
    for sample in tqdm(data, desc="Evaluating"):
        result = evaluate_single_veo_sample(
            agent=agent,
            sample=sample,
    ...
    tool_usage_rate = veo_called_count / n_success if n_success else 0.0
    ...
    for r in successful_results:
        ...
        if task not in task_stats:
            task_stats[task] = {"total": 0, "veo_called": 0, "generation_success": 0}
        task_stats[task]["total"] += 1
        if r.get("veo_called"):
            task_stats[task]["veo_called"] += 1
        if r.get("generation_success"):
            task_stats[task]["generation_success"] += 1

    for task in task_stats:
        t = task_stats[task]
        t["tool_usage_rate"] = t["veo_called"] / t["total"] if t["total"] else 0.0
    ...
    return {
        ...
        "veo_called_count": veo_called_count,
...
def print_veo_evaluation_results(results: Dict[str, Any]):
    """Print a formatted summary of Veo evaluation results."""
    print(f"\n{'='*70}")
    print(f"Veo Evaluation Results — {results['config_name']}")
    print(f"{'='*70}")
    print(f"Model:                    {results['model']}")
    print(f"Total samples:            {results['total_samples']}")
    print(f"Successfully processed:   {results['successful_samples']}")
    print(f"Failed to process:        {results['failed_samples']}")
    print(f"Veo tool called:          {results['veo_called_count']}  "
    ...
    if results.get("task_statistics"):
        print(f"\n{'Task Breakdown':-<50}")
        print(f"{'Task':<15} {'Total':<8} {'Veo Called':<12} {'Gen Success':<14} "
              f"{'Usage Rate':<13} {'Success Rate'}")
        print("-" * 70)
        for task, stats in results["task_statistics"].items():
            print(
                f"{task:<15} {stats['total']:<8} {stats['veo_called']:<12} "
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate SPAgent with the Veo video generation tool."
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="dataset/veo_eval_data.jsonl",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel worker threads (default: 1; Veo API has rate limits).",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--use_mock",
        action="store_true",
        help="Use mock Veo service instead of the real Gemini API.",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    for config_name, tools in configs_to_run.items():
        ...
        print_veo_evaluation_results(results)

    # Save results to JSON
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = (
        f"veo_evaluation_results_{args.model.replace('-', '_')}_{timestamp}.json"
    )
