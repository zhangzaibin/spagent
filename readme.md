...
| **OneFormer** | 2D | Universal Image Segmentation | Local / Server (port 20035) | Semantic, instance, and panoptic segmentation from one model; loaded via HuggingFace Transformers (auto-download, no manual checkpoint needed); default model: `shi-labs/oneformer_ade20k_swin_large` |
...
#### OneFormer (local or server)

OneFormer is loaded directly via HuggingFace Transformers — no manual checkpoint download needed. The model is auto-downloaded on first use (~1.5 GB).

```bash
pip install transformers Pillow
# Optional: use a HuggingFace mirror if the Hub is slow
